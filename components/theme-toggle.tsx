...
export const ThemeToggle: React.FC<ThemeToggleProps> = ({ className = '' }) => {
  ...
  return (
    <button
      ...
      className={`relative inline-flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 rounded-lg sm:rounded-xl transition-all duration-300 cursor-pointer select-none group border shrink-0 ${
        isDark
    ...
    >
      <div className="relative w-3.5 h-3.5 sm:w-4 sm:h-4 flex items-center justify-center">
        ...
        <Sun
          className={`w-3.5 h-3.5 sm:w-4 sm:h-4 absolute transition-all duration-300 transform ${
            isDark
